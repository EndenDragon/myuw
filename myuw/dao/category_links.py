...
from myuw.dao.enrollment import is_nm
import csv
import os
...
def get_links_for_category(search_category_id, request):
    return _get_links_by_category_and_campus(request,
                                             search_category_id,
                                             get_base_campus(request),
                                             get_all_affiliations(request))


def _get_links_by_category_and_campus(request,
                                      search_category_id,
                                      campus,
                                      affiliations):
    selected_links = []
    ...
    for link in all_links:
        ...
        if link.campus_matched(campus) and\
                is_nm(request) and link.for_nm() and\
                _pce_match(link, affiliations):
            selected_links.append(link)
            continue
